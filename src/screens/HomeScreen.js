...
export default function HomeScreen({ navigation }) {
  ...
  const handleAddTask = () => {
    ...
    Alert.alert('Task Added', `Task "${newTaskTitle}" has been added.`);
  };
  ...
  return (
    <Container>
      ...
      <QuoteButton onPress={handleQuoteButton}>
       ...
       <ObjectivButton /* onPress={handleQuoteButton} */>
      ...
      <View style={{ height: 225 }}> 
        <FlatList
          data={sortedTasks}
          renderItem={({ item }) => (
            <TaskItem completed={item.completed} onLongPress={() => handleLongPress(item.id)}>
...
const ObjectivButton = styled.TouchableOpacity`
  flex-direction: row;
  justify-content: center;
  align-items: center;
  padding: 50px;
